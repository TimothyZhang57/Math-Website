//Controller for Logarithmic and Exponential Functions Page
//Refer to comments on Measurements Ctrl 
angular.module('LogAndExponentCtrl', []).controller('LogAndExponentController', function($scope, $http,$window, $location) {
	$http.get('/api/ExponentialAndLogarithmic').
   ...
   then(function(response) {
	  ...
	  $scope.Q1C1 = response.data[1].Choices.substring(0, response.data[1].Choices.indexOf(","));
	  $scope.Q1C2 = response.data[1].Choices.substring(response.data[1].Choices.indexOf(",")+1, response.data[1].Choices.indexOf(" "));
	  $scope.Q1C3 = response.data[1].Choices.substring(response.data[1].Choices.indexOf(" ")+1);
