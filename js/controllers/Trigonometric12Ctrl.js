//Controller for the grade 12 Trigonmetric functions Page
//Refer to comments on Measurements Ctrl 
angular.module('Trigonometric12Ctrl', []).controller('Trigonometric12Controller', function($scope, $http,$window, $location) {
	$http.get('/api/TrigonometricFunctions12').
   ...
   then(function(response) {
	  ...
	  $scope.Q3C1 = response.data[1].Choices.substring(0, response.data[1].Choices.indexOf(","));
	  $scope.Q3C2 = response.data[1].Choices.substring(response.data[1].Choices.indexOf(",")+1, response.data[1].Choices.indexOf(" "));
	  $scope.Q3C3 = response.data[1].Choices.substring(response.data[1].Choices.indexOf(" ")+1);
